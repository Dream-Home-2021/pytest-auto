...
def work_login_init():
    ...
    根据实际的数据格式修改所有，不一定从登录获取。如果能直接网址获取token就索要token，下面的代码按情况修改
    ...
    res.json()
    下面代码可用可不用，看具体情况
    CacheHandler.update_cache的缓存设计原理是一个字典，存放了很多数据，在运行任何测试用例回先更新缓存
    调用方法（from utils.requests_tool.request_control import RequestControl）
    在调用时创建一个新的key叫'login_cookie'，
    该key的值是cookies，这里的变量要跟随数据情况灵活变化
    """
